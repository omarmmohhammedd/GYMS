...
exports.addClub = asyncHandler(async (req, res, next) => {
    const { name, email, password, city, location, description, gender } = req.body
    let imgs_path =[]
    if (req.files) {
    ...
    await User.findOne({ email }).then(async user => {
        ...
        await Club.create({ name, city, location, description, gender, images: imgs_path }).then(async club => {
            await User.create({ email, password: await bcrypt.hash(password, 10), role: "club",club: club.id})
